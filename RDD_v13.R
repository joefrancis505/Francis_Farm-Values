...
perform_rdd_50points <- function(data, point, include_slope = FALSE, exclude_border = FALSE, include_enslaved = FALSE) {
...
perform_rdd_robustness <- function(data, include_slope = FALSE, exclude_border = FALSE, include_enslaved = FALSE,
                                   bw_method = "mserd", kernel = "triangular", exclude_ohio_virginia = FALSE) {
...
run_analysis <- function(year) {
  ...
  for (spec in specifications) {
    ...
    write_csv(results, paste0("RDD/CSV/", year, "_farmv", spec$suffix, ".csv"))
    st_write(results_sf, paste0("RDD/Geopackage/", year, "_farmv", spec$suffix, ".gpkg"), delete_layer = TRUE)
  }
