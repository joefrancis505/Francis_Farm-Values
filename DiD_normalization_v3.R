...
perform_processing <- function() {
  ...
  census_data <- read_csv("DiD_database.csv", show_col_types = FALSE)
  ...
  for (year in years) {
    ...
    output_file <- sprintf("DiD/%d_processed.csv", year)
...
compile_panel_data <- function() {
  ...
  panel_data <- map_dfr(years, function(year) {
    ...
    file_name <- sprintf("DiD/%d_processed.csv", year)
  ...
  output_file <- "DiD_panel_data.csv"
...
main <- function() {
  ...
  panel_data <- read_csv("DiD_panel_data.csv", show_col_types = FALSE)
